...
export default function FlashSaleSection() {
  ...
  return (
    <section className="py-16 bg-green-900 text-white relative">
      <div className="max-w-7xl mx-auto px-4 md:px-12">
        ...
        <div className="relative">
          ...
          >
            {categories.map((cat, index) => (
              <SwiperSlide key={index}>
                <div className="flex flex-col md:flex-row items-center md:items-start bg-green-100/10 rounded-xl shadow-lg p-6 md:p-8 relative">
                  {/* Left Image */}
                  ...
                  <div className="w-full md:w-1/2 mt-4 md:mt-0 md:ml-6 flex flex-col justify-center">
                    ...
                    <p className="text-green-200 mb-4">{cat.products} Products Available</p>
          ...
          <button
            ...
            className="absolute top-1/2 left-0 transform -translate-y-1/2 z-10 bg-white/80 text-green-900 w-10 h-10 rounded-full flex items-center justify-center shadow hover:bg-white transition"
          >
            &#8592;
          </button>
          <button
            onClick={() => swiperRef.current?.slideNext()}
            className="absolute top-1/2 right-0 transform -translate-y-1/2 z-10 bg-white/80 text-green-900 w-10 h-10 rounded-full flex items-center justify-center shadow hover:bg-white transition"
          >
            &#8594;
          </button>
        </div>
